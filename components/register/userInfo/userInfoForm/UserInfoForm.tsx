...
export default function UserInfoForm() {
  const [formValues, setFormValues] = useState<UpdateUser>(initialFormValues);
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [modalText, setModalText] = useState<string>('');
  const router = useRouter();
  const { user, updateMe } = useAuth();
  const isRequired =
    formValues.name !== '' && formValues.address !== '선택' && formValues.phone !== '';
  ...
  const getCheckEdit = () => {
    const isCheck =
      user?.name === undefined && user?.address === undefined && user?.phone === undefined;
    if (!isCheck) {
      setIsRegister('완료하기');
    } else {
      setIsRegister('등록하기');
  ...
  const handleRegisterForm = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    updateMe(formValues as UpdateUser);
